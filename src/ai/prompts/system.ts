...
6. **名称完全一致**才视为引用已有节点；名称不在对照表中的必须**新建**节点（如用户要「风符文」而画布只有「火符文」，不得连到火符文或复用其 id）
7. 新建节点 id 基于 name 生成英文 snake_case，加随机后缀避免冲突
8. 仅输出 JSON，不要 markdown 代码块外的任何文字
9. 生成图/补全/修改时：必须输出一个 JSON 对象，包含 nodes 数组、edges 数组、explanation 字符串
10. 修改已有节点时：先理解用户中文描述指哪个「名称」，再查对照表得到 id；nodes 中 id 必须与对照表一致，只更新需要变更的字段
11. 用户指令中的节点指代以中文名称为准，不要要求用户说 id，也不要仅凭 id 猜测意图
12. 用户提到自定义类型（如「资源」「符文」）时，使用对应 custom_* type，不要擅自改成内置 ability/event 等
13. 用户提到自定义关系（如「包含」「产出」）时，使用对应 rel_* relationType，不要擅自改成内置 requires/triggers 等
14. 用户描述「A 解锁/连接 B」且 B 的名称不在对照表：nodes 必须包含 B 的新节点，edges 的 from/to 指向正确 id（含新建节点的新 id）`
...
注意：
- 用户若提到已有节点的中文名称（与对照表**完全一致**），edges/nodes 中必须使用对照表里的 id，勿重复创建
- 用户提到的名称**不在**对照表中时，必须在 nodes 里**新建**该节点，禁止连到名称相似但不同的已有节点（如「风符文」≠「火符文」）
- 用户自定义类型请用 custom_* / rel_* 的 type 值`
}
...
5. 新增节点：type 从「可用节点类型」选择；id 用英文 snake_case；连线 from/to 必须用对照表 id 或本次新建的 id
6. 连线 relationType 须从「可用关系类型」选择（含 rel_* 自定义）
7. **新建 vs 引用**：仅当名称与对照表**完全一致**时才引用已有 id。名称不在表中的（如「风符文」而画布只有「火符文」）必须在 nodes 中新建，禁止猜测连到相似节点
8. 用户描述「A 解锁/连接 B」且 B 不在对照表：nodes 须含 B，edges 指向 B 的新 id
